...
OMP_NUM_THREADS=1 xvfb-run -a python -m cleanrl_utils.benchmark \
    ...
    --command "uv run python cleanrl/vmpo_continuous_action.py --no_cuda --track --capture_video" \
    --num-seeds 3 \
    --workers 9 \
...
OMP_NUM_THREADS=1 xvfb-run -a uv run python -m cleanrl_utils.benchmark \
    ...
    --command "uv run python cleanrl/vmpo_continuous_action.py --exp-name vmpo_continuous_action_8M  --total-timesteps 8000000 --no_cuda --track" \
    --num-seeds 10 \
    --workers 9 \
    --slurm-gpus-per-task 1 \
    --slurm-ntasks 1 \
